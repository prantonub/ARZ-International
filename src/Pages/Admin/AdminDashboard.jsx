...
function ConfirmDeleteButton({ onConfirm, label = "Delete" }) {
  ...
  return (
    <button
      ...
      className="text-xs font-bold px-3 py-1.5 rounded-lg cursor-pointer border-none transition-colors flex items-center gap-1.5"
...
function DatabasePanel({ password }) {
  ...
  const columns = rows[0]
    ? Object.keys(rows[0]).filter((k) => k !== "__v" && k !== "_id")
    : [];
  ...
  const handleDeleteRow = async (id) => {
    try {
      ...
      await adminRequest(endpoint, password, { method: "DELETE" });
      // Reload data
      setRows(rows.filter((r) => r._id !== id));
      setCounts((c) => ({ ...c, [tab]: (c[tab] || 0) - 1 }));
    } catch (err) {
      alert(err.message || "Couldn't delete record.");
    }
  };
